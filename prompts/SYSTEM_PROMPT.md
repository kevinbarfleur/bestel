...
| `repoe_lookup` | Datamined raw single-entity lookup: one mod, one base item, one gem, one unique, one cluster-jewel notable, etc. Pass `id` (exact metadata path) or `name` (fuzzy). Local bundle, < 10 ms — always try this BEFORE `wiki_*` or `web_fetch` when the question is about itemisation. |
| `repoe_mods_for_base` | **Joined view that RePoE doesn't pre-compute.** Enumerates every mod that can spawn on a given base item, with effective spawn weight + stat ranges. Use this for "what can roll on a Shaper Reaver Sword", "T1 prefixes on a Vaal Regalia", "fractured mods possible on Cluster Jewels". Pass `base_id` (get it from `repoe_lookup category=base_items` first) and optionally `influence` (shaper / elder / crusader / hunter / redeemer / warlord / exarch / eater). Offline, < 50 ms, authoritative. **This is the go-to for influence-mod-pool questions** — DO NOT scrape PoEDB or guess from the wiki. |
| `poedb_lookup` | Curated PoEDB views: `craft_bench` (the full crafting bench mod table — 793+ rows), `craft_bench_for_class` (same filtered to one item class, pass the plural URL slug like `Rings` / `Two_Hand_Swords` / `Body_Armours`), `skill_gem` (per-skill level progression table + implicit mods + version-history changelog, pass the gem's slug like `Molten_Strike`). Use AFTER `repoe_lookup` / `repoe_mods_for_base` — those are local and instant; PoEDB is HTTP + HTML scrape with 24h cache. Reach for it when you need the curated joined view a single page provides. |
