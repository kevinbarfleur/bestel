...
pub async fn dispatch_get_active_build_sheet(
    ...
    Ok(json!({
        ...
        "next_required_action": if stale {
            "STALE_SHEET: A sheet exists but the PoB hash differs since authoring. Read the sheet anyway — intent, archetype, defining items, and known gaps are authored decisions that don't go stale when gear shifts ; only the numbers age. Open your answer with one short sentence acknowledging the drift (e.g. \"Your sheet was authored before your last gear pass — the framing and item picks still hold, but I'm taking the numbers off the live PoB.\"). Cite sections.intent / archetype / items for context. For numerical claims (DPS, EHP, max-hit, resists, regen), DO NOT use the sheet's stored numbers — re-derive from the live PoB via pob_calc and present those. End with `read_from_sheet · key1 · key2` italic caption listing the sections you cited (intent / archetype / items, never numbers). Do NOT call sheet_open_interview to refresh — the user has a refresh button in the UI and triggers it explicitly."
        } else {
